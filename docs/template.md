...
4. **Rewrites `package.json`** (with a project name, also drops Fastra's `repository`, `keywords`, `author`, and `license`; the `LICENSE` file is removed with the setup tool): removes dependencies and scripts owned only by unselected options, sets the selected options' scripts, and removes the setup tool.
